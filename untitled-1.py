...
def computePMIIA(G, ISv, v, theta, S, Ep):
	...
	crossing_edges = set([in_edge for in_edge in G.in_edges([v]) if in_edge[0] not in ISv + [v]]) # [(u, v), ...]
	edge_weights = dict()
...
def PMIA(G, k, theta, Ep):
	...
	# initialization
	S = []
	IncInf = dict(zip(G.nodes(), [0]*len(G))) # zip 返回 [(v0, 0), (v1, 0), (v2, 0), ...]，dict 返回 {v0 : 0, v1 : 0, v2 : 0, ...}
